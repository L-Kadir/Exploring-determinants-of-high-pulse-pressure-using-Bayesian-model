...
model{
  for (i in 1:Ntotal){
    zy[i] ~ dt(zbeta0  + zbeta1*z1[i] + zbeta2*z2[i] + zbeta3*z3[i] + zbeta4*z2[i]*z3[i] , 1/zsigma^2,nu)
  }
  # Vague priors
...
plot(x2, y,ylim=range(c(15,150)),xlab="x2 - Age", ylab="y - PP", 
     main = "Posterior Predictive Check",col="black", pch=20, cex = 1)
for (i in 1:mm){
  y.prd2 <- b0[i*10] + b1[i*50]*mean(x1) + b2[i*50]*xp2 + b3[i*50]*mean(x3)
