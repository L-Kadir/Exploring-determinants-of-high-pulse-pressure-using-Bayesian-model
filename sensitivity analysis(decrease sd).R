...
model{
  for (i in 1:Ntotal){
    zy[i] ~ dt(zbeta0  + zbeta1*z1[i] + zbeta2*z2[i] + zbeta3*z3[i], 1/zsigma^2,nu)
  }
  # Vague priors
  zbeta0 ~ dnorm(0,1/0.5^2)  # mean 0, sd = 0.5
  zbeta1 ~ dt(0, 1/sigmaBeta^2,1) 
  zbeta2 ~ dt(0, 1/sigmaBeta^2,1)
  zbeta3 ~ dt(0, 1/sigmaBeta^2,1)
  zsigma ~ dunif(0.00001,.99999)
  nu ~ dexp(.0333) # mean 30, sd = 30
  sigmaBeta ~ dgamma(2.618, 0.5) # mode 1, sd 0.5
...
myinits <-list(list(zbeta0=rnorm(1,0,1), zbeta1=rnorm(1,0,1), zbeta2=rnorm(1,0,1),zbeta3=rnorm(1,0,1),
                    zsigma=runif(1),nu = runif(1),sigmaBeta = runif(1)),
...
out <- run.jags(model="MultipleRegression-M1h.txt",data=dataList, inits=myinits, n.chains=3,
                ...
                monitor=c("zbeta0","zbeta1","zbeta2","zbeta3","beta0","betaSbp","betaAge","betaChol","nu",
                          "sigmaBeta","zsigma","DIC"))
print(out)
plot(out)
