...
model{
  for (i in 1:Ntotal){
    zy[i] ~ dnorm(zbeta0  + zbeta1*z1[i] + zbeta2*z2[i] + zbeta3*z3[i] + zbeta4*z2[i]*z2[i] , 1/zsigma^2)
  }
  # Vague priors
...
myinits <-list(list(zbeta0=rnorm(1,0,1), zbeta1=rnorm(1,0,1), zbeta2=rnorm(1,0,1),zbeta3=rnorm(1,0,1),
                    zbeta4=rnorm(1,0,1),zsigma=runif(1)),
...
out <- run.jags(model="MultipleRegression-M1h.txt",data=dataList, inits=myinits, n.chains=3,
                ...
                monitor=c("zbeta0","zbeta1","zbeta2","zbeta3","zbeta4","zsigma","DIC"))
print(out)
plot(out)
