...
model{
  for (i in 1:Ntotal){
    zy[i] ~ dt(zbeta0  + zbeta1*z1[i] + zbeta2*z2[i] + zbeta3*z3[i] + zbeta4*z2[i]*z2[i] , 1/zsigma^2,nu)
  }
  # Vague priors
...
myinits <-list(list(zbeta0=rnorm(1,0,1), zbeta1=rnorm(1,0,1), zbeta2=rnorm(1,0,1),zbeta3=rnorm(1,0,1),
                    zbeta4=rnorm(1,0,1),zsigma=runif(1),nu = runif(1)),
...
out <- run.jags(model="MultipleRegression-M1h.txt",data=dataList, inits=myinits, n.chains=3,
                ...
                monitor=c("zbeta0","zbeta1","zbeta2","zbeta3","zbeta4","nu","zsigma","DIC"))
print(out)
plot(out)

# Extract MCMC samples
aa0 <- as.mcmc.list(out, vars="zbeta0")
b0 <- as.numeric(unlist(aa1[,1]))
aa1 <- as.mcmc.list(out, vars="zbeta1")
b1 <- as.numeric(unlist(aa1[,1]))
aa2 <- as.mcmc.list(out, vars="zbeta2")
b2 <- as.numeric(unlist(aa2[,1]))
aa3 <- as.mcmc.list(out, vars="zbeta3")
b3 <- as.numeric(unlist(aa3[,1]))
aa4 <- as.mcmc.list(out, vars="zbeta4")
b4 <- as.numeric(unlist(aa3[,1]))
...
plot(x2, y,ylim=range(c(15,150)),xlab="x2 - Age", ylab="y - PP", 
     main = "Posterior Predictive Check",col="black", pch=20, cex = 1)
for (i in 1:mm){
  y.prd2 <- b0[i*10] + b1[i*10]*mean(x1) + b2[i*10]*xp2 + b3[i*10]*mean(x3)
...
plot(x3, y,ylim=range(c(15,150)),xlab="x3 - Chol", ylab="y - PP", 
     main = "Posterior Predictive Check",col="black", pch=20, cex = 1)
for (i in 1:mm){
  y.prd3 <- b0[i*50] + b1[i*50]*mean(x1) + b2[i*50]*mean(x2) + b2[i*50]*xp3
...
pvaf <- rep(0,mmm)
for (i in 1:mmm){
  y.prd <- b0[i*50] + b1[i*50]*x1 + b2[i*100]*x2 + b3[i*50]*x3 + b4[i*50]*x3*x2
  pvaf[i] <- 1-sum((y - y.prd)^2)/sum((y - mean(y))^2)
}
cat('Mean percent variance accounted for (PVAF): ', mean(pvaf))
# Normality check for residuals
residu <- mean(b0)+mean(b1)*x1+mean(b2)*x2 + mean(b3)*x3 + mean(b4)*x3*x4 - y
